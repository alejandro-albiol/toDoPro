...
export class JwtService {
    ...
    constructor() {
        if (!process.env.JWT_SECRET) {
            throw new InvalidJwtSecretException('JWT_SECRET is not defined in environment variables');
        }
    ...
    verifyToken(token: string): { userId: string; username: string } {
        try {
            return jwt.verify(token, this.secretKey) as { userId: string; username: string };
        } catch (error) {
